...
for(kdonor in c(1:3)){
  # for(k in 1){
    ...
    ###
    # library(pracma)  # for the gramSchmidt function
    
    QR <- qr(a_)
    a_ortho = qr.Q(QR)
    # a_ortho = scale(a_ortho)
    ...
    layers_pred_sampleTmp = walktrap_clustering(clusternum=ifelse(kdonor == 2, 5, 7), latent_dat=t(a_ortho[,]), knearest=70)
    which_na = which(is.na(layers))
    print(ARI(layers_pred_sampleTmp[-which_na], layers[-which_na]))
    
    
    saveRDS(layers_pred_sampleTmp, file = paste0("layers_pred_withinEachsample_sample_",ksample,"_scaleV2_3samples_010induced_set",set,"_ortho.rds"))
...
for(kdonor in c(1:3)){
  # for(k in 1){
    ...
    saveRDS(clusterlabel_refine, file = paste0("layers_pred_withinEachsample_sample_",ksample,"_refined_scaleV2_3samples_010induced_set",set,"_ortho.rds"))
    
  # }
}
saveRDS(ARI_vec, file = paste0("ARI_vec_sample1_5_9_predLayers_mnsf_ClusterWithinEachSample_mnsfrefined_scaleV2_3samples_034induced_set",set,"_ortho.rds"))
# set2
# set3
# set4
