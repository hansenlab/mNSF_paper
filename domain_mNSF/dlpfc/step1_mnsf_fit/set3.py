...
for kdonor in range(0,nsample):
	Y=pd.read_csv(path.join('//dcs04/hansen/data/ywang/revision/domain//Y_sample'+str(kdonor*4  + 3)+'_svg.csv'))
	X=pd.read_csv(path.join('//dcs04/hansen/data/ywang/revision/domain//X_sample'+str(kdonor*4  + 3)+'_svg.csv'))
...
for kdonor in range(0,nsample):
	random.seed(111)
	ninduced=round(list_D[kdonor]['X'].shape[0] * 0.35)
	random.seed(222)
	print(ninduced)
	D=list_D[kdonor]
	rd_ = random.sample(range(0, D['X'].shape[0]), ninduced)
	list_D[kdonor]["Z"]=D['X'][rd_ ,:]
...
process_multiSample.save_object(list_fit, 'list_fit_nb_3samples_szMean_L10_fullData_svg_set3.pkl') 



########################################################################
with open( 'list_fit_nb_3samples_szMean_L10_fullData_svg_set3.pkl', 'rb') as inp:
...
#Wdf.to_csv(path.join("loadings_spde_nb_szMean_L10_3samples_0.35Induced_svgs_set3.csv"))
...
for k in range(0,nsample):
	...
	Factors_df.to_csv(path.join(dir_output,"factors_nb_szMean_sample_s"+str(k+1)+"_L10_3samples_0.35Induced_svgs_set3.csv"))
