...
for kdonor in range(0,nsample):
	Y=pd.read_csv(path.join('//dcs04/hansen/data/ywang/revision/domain//Y_donor'+str(kdonor  + 1)+'_svg.csv'))
	X=pd.read_csv(path.join('//dcs04/hansen/data/ywang/revision/domain//X_donor'+str(kdonor  + 1)+'_svg.csv'))
...
process_multiSample.save_object(list_fit, 'list_fit_nb_3samples_szMean_L10_fullData_svg.pkl') 



########################################################################
with open( 'list_fit_nb_3samples_szMean_L10_fullData_svg.pkl', 'rb') as inp:
...
Wdf.to_csv(path.join("loadings_spde_nb_szMean_L10_3samples_0.35Induced_svgs.csv"))
...
for k in range(0,nsample):
	...
	Factors_df.to_csv(path.join(dir_output,"factors_nb_szMean_sample_s"+str(k+1)+"_L10_3samples_0.35Induced_svgs.csv"))
