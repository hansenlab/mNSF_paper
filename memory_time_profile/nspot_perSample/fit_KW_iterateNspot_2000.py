...
output_memory_list = []
output_memory_columns = ["nspot", "peak_before_first_iteration", "peak_after_training", "runtime"]

memory_outfile = "memoryUsage_mouseSagittal_iterNspot_runSeperately.csv"
...
for ksample in range(0,nsample):
	X=pd.read_csv(path.join(dir_data,'X_forMemTimeProfile.csv'))
	Y=pd.read_csv(path.join(dir_data,'Y_500genes_forMemTimeProfile.csv'))
	Y=Y.iloc[0:nspot,:]
	X=X.iloc[0:nspot,:]
...
output_memory_list.append(nspot)
