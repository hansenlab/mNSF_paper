...
for(kdonor in c(1:3)){
  ...
  # }else{
    a_=read.csv(paste0("//dcs04/hansen/data/ywang/ST/DLPFC/PASTE_out_keepAll_scTransform//factors_nb_szMean_sample_s",
                       ksample,"_L",L,"_fullData_010Induced.csv"),header=T)
  # }
  a = a_[,-1]
...
for(kdonor in c(1:3)){
  for(k in 1:4){
    ...
    saveRDS(layers_pred_sampleTmp, file = paste0("layers_pred_withinEachsample_sample_",ksample,"_L",L,"_scaleV2_010Induced.rds"))
...
for(kdonor in c(1:3)){
  ...
  layers_pred_sampleTmp = readRDS(paste0("layers_pred_withinEachsample_sample_",ksample,"_L",L,"_scaleV2_010Induced.rds"))
  ...
  saveRDS(clusterlabel_refine, file = paste0("layers_pred_withinEachsample_sample_",ksample,"_L",L,"_refined_scaleV2_010Induced.rds"))
  
}
}
saveRDS(ARI_vec,file = paste0("ARI_vec_sample1_5_9_predLayers_mnsf_ClusterWithinEachSample_mnsfrefined__L",L,"_scaleV2_010Induced.rds"))
