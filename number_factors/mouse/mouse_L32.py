...
for ksample in range(0,nsample):
	random.seed(222)
	ninduced=round(list_D[ksample]['X'].shape[0] * 0.10)
	print(ninduced)
	D=list_D[ksample]
...
process_multiSample.save_object(list_fit, 'list_fit_500selectedFeatures_dev_interpolated_10percent_szMean_L'+str(L)+'.pkl') 
...
Wdf.to_csv(path.join("loadings_spde_L20_500selectedFeatures_dev_interpolated_10percent_szMean_L"+str(L)+".csv"))
...
for k in range(0,nsample):
	...
	Factors_df.to_csv(path.join("factors_sample"+str(k+1)+"_500selectedFeatures_dev_interpolated_10percent_szMean_L"+str(L)+".csv"))
