...
for ksample in range(0,nsample):
	ad=read_h5ad(path.join("ggblocks_lr_"+str(ksample+1)+"_sizeDif.h5ad"))
	...
	X_df.to_csv(path.join("X_sample"+str(ksample+1)+"_sizeDif.csv"))
...
process_multiSample.save_object(list_fit, path.join('sim_rotated_list_fit_sizeDif.pkl') )
...
Wdf.to_csv(path.join("sim_rotated_loadings_spde_sizeDif.csv"))
...
for k in range(0,nsample):
	...
	Factors_df.to_csv(path.join("sim_rotated_factors_sample"+str(k+1)+"_sizeDif.csv"))
...
for ksample in range(0,nsample):
	...
	fig.savefig(path.join("sim_rotated_factors_sample"+str(ksample+1)+"_sizeDif.png"),bbox_inches='tight')
