...
for ksample in range(0,nsample):
	ad=read_h5ad(path.join("ggblocks_lr_"+str(ksample+1)+"_noiseDif.h5ad"))
	...
	X_df.to_csv(path.join("X_sample"+str(ksample+1)+"_noiseDif.csv"))
...
process_multiSample.save_object(list_fit, path.join('sim_rotated_list_fit_noiseDif.pkl') )
...
Wdf.to_csv(path.join("sim_rotated_loadings_spde_noiseDif.csv"))
...
for k in range(0,nsample):
	...
	Factors_df.to_csv(path.join("sim_rotated_factors_sample"+str(k+1)+"_noiseDif.csv"))
...
for ksample in range(0,nsample):
	...
	fig.savefig(path.join("sim_rotated_factors_sample"+str(ksample+1)+"_noiseDif.png"),bbox_inches='tight')
