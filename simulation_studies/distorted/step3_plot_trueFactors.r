...
{
  sample_=1
  a_true_s1=read.csv(paste0("ggblocks_lr_1_", type, "_sample",sample_,".csv"))
  a_true_s1=a_true_s1[,-1]
  X_s1 = read.csv(paste0("X_sample",1,"_",type,".csv"))
  X_s1 = X_s1[,-1]
  
  sample_=2
  a_true_s2=read.csv(paste0("ggblocks_lr_1_", type, "_sample",sample_,".csv"))
  a_true_s2=a_true_s2[,-1]
  X_s2 = read.csv(paste0("X_sample",2,"_",type,".csv"))
  X_s2 = X_s2[,-1]
  
  valueRange_ = range(c(as.numeric(data.matrix(a_true_s1)),as.numeric(data.matrix(a_true_s2))))
  ...
  for (sample_ in 1:2){
    ...
    for (i in 1:4) {
      ...
      plot_tmp =  ggplot(df_tmp,aes(x=imagecol,y=imagerow,fill=fill_tmp)) +
        ...
        geom_point(shape = 22,  size = 6, stroke = NA)+
    ...
    pdf(paste0("trueFactors_sim_",type,"_sample",sample_,".pdf"),height=2.6,width=18/6*4.5)
