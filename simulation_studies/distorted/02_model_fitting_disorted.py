...
########################################################################
mpth = path.join("models_distorted")
misc.mkdir_p(mpth)
...
for ksample in range(0,nsample):
	ad=read_h5ad(path.join("ggblocks_lr_"+str(ksample+1)+"_distorted.h5ad"))
	...
	list_D.append(D)
	list_X.append(D["X"])
	X_df = pd.DataFrame(ad.obsm['spatial'] ) 
	X_df.to_csv(path.join("X_sample"+str(ksample+1)+"_distorted.csv"))
...
process_multiSample.save_object(list_fit, path.join('sim_rotated_list_fit_distorted.pkl') )
...
Wdf.to_csv(path.join("sim_rotated_loadings_spde_distorted.csv"))
...
for k in range(0,nsample):
	...
	Factors_df.to_csv(path.join("sim_rotated_factors_sample"+str(k+1)+"_distorted.csv"))
...
for ksample in range(0,nsample):
	...
	fig.savefig(path.join("sim_rotated_factors_sample"+str(ksample+1)+"_distorted.png"),bbox_inches='tight')
